...
@section('content')
    ...
    <div class="card table-card">
        <!-- Search Bar -->
        @include('_admin.users.partials.search', ['placeholder' => 'nome, email ou localização...'])
        ...
        <div class="table-wrapper">
            <table>
                ...
                <tbody>
                    @forelse ($items as $item)
                        <tr>
                            ...
                            <td class="actions align-right">
                                <a class="btn ghost small action-btn" href="{{ route('admin.users.show', $item->id) }}"><i
                                        class="ri-eye-line"></i><span>Ver</span></a>
                                <a class="btn secondary small action-btn" href="{{ route('admin.users.edit', $item->id) }}"><i
                                        class="ri-pencil-line"></i><span>Editar</span></a>
                                <form class="inline confirm-delete" method="POST"
                                    action="{{ route('admin.users.destroy', $item->id) }}">@csrf
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $columns->count() + 2 }}" class="muted">Ainda não existem registos.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
