...
@section('title', 'Ver Newsletter')
@section('content')
    <div class="page-header">
        <div>
            <p class="eyebrow">{{ $resource['title'] ?? '' }}</p>
            <h2>Newsletter #{{ $item->id }}</h2>
        </div>
    ...
    <div class="form-actions floating-actions">
        ...
        <a class="btn secondary btn-lg" href="{{ route('admin.newsletters.edit', $item->id) }}">
            <i class="ri-pencil-line"></i> Editar
        </a>
        <form method="POST" action="{{ route('admin.newsletters.destroy', $item->id) }}" class="confirm-delete">
            @csrf
            @method('DELETE')
