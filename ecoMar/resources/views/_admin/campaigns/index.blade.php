...
@section('content')
    <div class="page-header">
        ...
        <div class="header-actions">
            <a class="btn primary" href="{{ route('admin.campaigns.create') }}">
    ...
    <div class="card table-card">
        <!-- Search Bar -->
        @include('_admin.campaigns.partials.search', ['placeholder' => 'título, descrição ou país...'])
                    ...
                    @forelse ($items as $item)
                        <tr>
                            ...
                            <td class="actions align-right">
                                <a class="btn ghost small action-btn" href="{{ route('admin.campaigns.show', $item->id) }}"><i
                                        class="ri-eye-line"></i><span>Ver</span></a>
                                <a class="btn secondary small action-btn"
                                    href="{{ route('admin.campaigns.edit', $item->id) }}"><i
                                        class="ri-pencil-line"></i><span>Editar</span></a>
                                <form class="inline confirm-delete" method="POST"
                                    action="{{ route('admin.campaigns.destroy', $item->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn danger small action-btn" type="submit"><i
                                            class="ri-delete-bin-line"></i><span>Eliminar</span></button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ count($columns) + 2 }}" class="muted">Nenhuma campanha encontrada.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
