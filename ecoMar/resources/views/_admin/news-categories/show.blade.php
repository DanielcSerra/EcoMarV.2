...
@section('title', 'Ver Categoria de Notícia')

@section('content')
    <div class="page-header">
        <div>
            <p class="eyebrow">{{ $resource['title'] ?? '' }}</p>
            <h2>Categoria de Notícia #{{ $item->id }}</h2>
        </div>
    </div>
    ...
    <div class="form-actions floating-actions">
        ...
        <a class="btn secondary btn-lg" href="{{ route('admin.news-categories.edit', $item->id) }}">
            <i class="ri-pencil-line"></i> Editar
        </a>
        <form method="POST" action="{{ route('admin.news-categories.destroy', $item->id) }}" class="confirm-delete">
            @csrf
            @method('DELETE')
