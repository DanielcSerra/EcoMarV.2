...
@section('title', 'Ver Sugestão de Evento')
@section('content')
    <div class="page-header">
        <div>
            <p class="eyebrow">{{ $resource['title'] ?? '' }}</p>
            <h2>Sugestão de Evento #{{ $item->id }}</h2>
        </div>
    ...
    <div class="form-actions floating-actions">
        ...
        <a class="btn secondary btn-lg" href="{{ route('admin.event-suggestions.edit', $item->id) }}">
            <i class="ri-pencil-line"></i> Editar
        </a>
        <form method="POST" action="{{ route('admin.event-suggestions.destroy', $item->id) }}" class="confirm-delete">
            @csrf
            @method('DELETE')
