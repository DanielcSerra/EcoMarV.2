...
@section('title', 'Ver Categoria de Evento')
@section('content')
    <div class="page-header">
        <div>
            <p class="eyebrow">{{ $resource['title'] ?? '' }}</p>
            <h2>Categoria de Evento #{{ $item->id }}</h2>
        </div>
    ...
    <div class="form-actions floating-actions">
        <a class="btn ghost btn-lg" href="{{ route('admin.event-categories.index') }}">
            <i class="ri-arrow-left-line"></i> Voltar
        </a>
        <a class="btn secondary btn-lg" href="{{ route('admin.event-categories.edit', $item->id) }}">
            <i class="ri-pencil-line"></i> Editar
        </a>
        <form method="POST" action="{{ route('admin.event-categories.destroy', $item->id) }}" class="confirm-delete">
            @csrf
            @method('DELETE')
